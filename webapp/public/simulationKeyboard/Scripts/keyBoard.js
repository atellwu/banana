...
function drawKeyboard(type) {
	...
	$.each(keyboardNum, function(key, value) {
		if (value != "Backspace") {
			key = $('<div class="simpleKey" name="key" key="' + key
					+ '" value="' + value + '">' + value + '</div>');
			$("#keyboardNum").append(key);
		} else {
			key = $('<div class="backspaceKey" name="key"  key="' + key
					+ '" value="' + value + '">' + value + '</div>');
			$("#keyboardNum").append(key);
		}
	});
	...
	$.each(keyboardLetterQ, function(key, value) {
		key = $('<div class="simpleKey" name="key"  key="' + key + '" value="'
				+ value + '">' + value + '</div>');
	...
	$.each(keyboardLetterA, function(key, value) {
		if (value != "Caps Lock") {
			...
			key = $('<div class="capslockKey" name="key"  key="' + key
					+ '" value="' + value + '">' + value + '</div>');
			$("#keyboardLetterA").append(key);
		}
	});
	...
	$.each(keyboardLetterZ, function(key, value) {
		if (value != "Shift") {
			...
			key = $('<div class="shiftKey" name="key"  key="' + key
					+ '" value="' + value + '">' + value + '</div>');
			$("#keyboardLetterA").append(key);
		}
	});
	...
	$.each(keyboardSpaceBar, function(key, value) {
		if (value != "Space") {
			...
			key = $('<div class="spaceKey" name="key"  key="' + key
					+ '" value="' + value + '">' + value + '</div>');
			$("#keyboardSpaceBar").append(key);
		}
	});
					...
					function() {
						...
						case "Caps Lock":
							$("#capsLock")
									.attr(
											"checked",
											$("#capsLock").attr("checked") == true ? false
													: true);
							$("#capsLock").attr("checked") == true ? drawKeyboard("upper")
									: drawKeyboard("lower");
							$("#shift").attr("checked", false)
							break;
						case "Space":
...
$(function() {
	var divKeyBoard = '<div id="keyboard" class="keyboard">'
			+ '<div id="keyboardHead"><div>'
			+ '<input id="shift" type="checkbox"/>Shift</div>'
			+ '<div><input id="capsLock" type="checkbox"/>Caps Lock</div>'
			+ '<div id="close" style="border:1px solid black; float:right; width:20px; height:20px; cursor:pointer;"><img src="Image/close.gif" style=" width:20px; height:20px"/></div></div>'
			+ '<div id="keyboardNum"></div>'
			+ '<div id="keyboardLetterQ"></div>'
			+ '<div id="keyboardLetterA"></div>'
			+ '<div id="keyboardLetterZ"></div>'
			+ '<div id="keyboardSpaceBar"></div></div>';
